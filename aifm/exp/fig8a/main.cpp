...
class FarMemTest {
private:
  constexpr static uint64_t kCacheSize = 800 * Region::kSize;
  ...
  template <typename T> T avg_of_last_N(std::vector<T> &vec, uint32_t n) {
    ...
    return std::accumulate(last_5.begin(), last_5.end(), 0.0) / last_5.size();
  }
    ...
    for (uint32_t tid = 0; tid < kNumMutatorThreads; tid++) {
      threads.emplace_back(rt::Thread([&, tid]() {
        ...
        while (1) {
          ...
          hopscotch_ptr->_get(*deref_scope_ptr, kKeyLen,
