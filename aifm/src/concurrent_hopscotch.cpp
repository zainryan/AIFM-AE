...
GenericConcurrentHopscotch::GenericConcurrentHopscotch(
  ...
  // Register notifier.
  FarMemManagerFactory::get()->register_notifier(
      ds_id, [&](Object obj) { this->notifier(obj); });
}
...
GenericConcurrentHopscotch::~GenericConcurrentHopscotch() {
  ...
  // Free remote data.
  FarMemManagerFactory::get()->deconstruct(ds_id_);
}

void GenericConcurrentHopscotch::do_notifier(NotifierMeta meta) {
  auto *bucket =
  ...
  if (likely(bucket->spin.TryLock())) {
    ...
    BUG_ON(!notifier_stash_.push_back(meta));
  }
}
...
void GenericConcurrentHopscotch::forward_get(uint8_t key_len,
  ...
  uint16_t data_len;
  FarMemManagerFactory::get()->read_object(ds_id_, key_len, key, &data_len,
                                           val);
  if (data_len) {
    *val_len = data_len - sizeof(NotifierMeta);
    _put(key_len, key, *val_len, val, /* remove_remote = */ false);
  } else {
    *val_len = 0;
  }
}
...
retry:
  ...
  while (bitmap) {
    ...
    if (obj.get_obj_id_len() == key_len) {
      ...
      if (strncmp(reinterpret_cast<const char *>(obj_val_ptr) + obj_data_len,
        ...
        if (unlikely(obj_data_len != val_len + sizeof(NotifierMeta))) {
          auto new_data_size = val_len + sizeof(NotifierMeta);
          ...
          auto new_meta = reinterpret_cast<NotifierMeta *>(
  ...
  while ((distance_to_orig_bucket = bucket_idx - orig_bucket_idx) >=
    ...
    for (distance = kNeighborhood - 1; distance > 0; distance--) {
      ...
      auto from_obj = Object(reinterpret_cast<uint64_t>(from_obj_val_ptr) -
                             Object::kHeaderSize);
      auto *from_meta = reinterpret_cast<NotifierMeta *>(
          const_cast<uint8_t *>(from_obj.get_obj_id()) - sizeof(NotifierMeta));
  ...
  if (!FarMemManagerFactory::get()->allocate_generic_unique_ptr_nb(
          ptr, ds_id_, sizeof(NotifierMeta) + val_len, key_len, key)) {
    bucket_lock_guard.reset();
    FarMemManagerFactory::get()->mutator_wait_for_gc_cache();
    goto retry;
  }
  ...
  assert(val_ptr);
  auto *meta = reinterpret_cast<NotifierMeta *>(
