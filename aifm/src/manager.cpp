...
FarMemManager::FarMemManager(uint64_t cache_size, uint64_t far_mem_size,
  ...
  memset(notifiers_, 0, sizeof(notifiers_));
...
void FarMemManager::swap_in(bool nt, FarMemPtrMeta *meta) {
...
void FarMemManager::swap_out(FarMemPtrMeta *meta, Object obj) {
  assert(preempt_enabled());
  ...
  if (meta->is_hot() && !meta->is_nt() && !ACCESS_ONCE(almost_empty)) {
      ...
      while (cur + Object::kHeaderSize < right) {
        ...
        if (!obj.is_freed()) {
          ...
          if (likely(!obj.is_freed())) {
            auto *meta = reinterpret_cast<FarMemPtrMeta *>(obj.get_ptr_addr());
            meta->set_evacuation();
          }
        }
        cur += obj.size();
...
#ifndef STW_GC
  store_release(&expected_status, flip_status(old_status));
