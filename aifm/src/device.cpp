...
FakeDevice::~FakeDevice() { deconstruct(kVanillaPtrDSID); }
...
void FakeDevice::deconstruct(uint8_t ds_id) { server_.deconstruct(ds_id); }
...
void TCPDevice::deconstruct(uint8_t ds_id) {
  auto remote_slave = shared_pool_.pop();
  _deconstruct(remote_slave, ds_id);
  shared_pool_.push(remote_slave);
}
...
// Response:
// |Ack (1B)|
void TCPDevice::_deconstruct(tcpconn_t *remote_slave, uint8_t ds_id) {
...
void TCPDevice::_compute(tcpconn_t *remote_slave, uint8_t opcode, uint8_t ds_id,
