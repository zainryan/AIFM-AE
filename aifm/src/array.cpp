...
GenericArray::GenericArray(FarMemManager *manager, uint32_t item_size,
                           uint64_t num_items)
    : kItemSize_(item_size), kNumItems_(num_items),
      induce_([](Index_t idx_0, Index_t idx_1) -> Pattern_t {
        return static_cast<Pattern_t>(idx_1) - static_cast<Pattern_t>(idx_0);
      }),
      infer_(
          [](Index_t idx, Index_t stride) -> uint64_t { return idx + stride; }),
      mapping_([&](uint64_t idx) {
        return idx < kNumItems_ ? &ptrs_[idx] : nullptr;
      }),
      prefetcher_(induce_, infer_, mapping_, item_size) {
  preempt_disable();
